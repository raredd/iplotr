...
This package reformulates the core of `qtlcharts` into more generic plotting functions for any type of data. Additionaly, the plotting functions here were designed to be used similarly to base R plotting functions by using familiar syntax and matching expected results.

Interactive plots are particularly useful for exploratory data analysis within R but may also be included in [R Markdown](http://rmarkdown.rstudio.com) documents. [R Markdown](http://rmarkdown.rstudio.com) is an extension of [Markdown](http://daringfireball.net/projects/markdown/), using [knitr](http://yihui.name/knitr/) to write simply-marked-up text with chunks of R code which is converted to an html file for viewing in a web browser.

For more information on R Markdown and knitr, see Karl's [knitr in a knutshell](http://kbroman.org/knitr_knutshell/) tutorial.

To include interactive plots within an R Markdown document, you just need to include the relevant calls to the R function within a code chunk.
...
with(
  mtcars, iscatter(wt, mpg, group = cyl, col = 1:3, labels = labels, cex = 5,
...
```{r}
v_labels <- list(' ' = rownames(ivolcano),
...
with(
  ivolcano, iscatter(logFC, -log10(pval), col = c('lightgrey','green'),
...
```{r}
with(mtcars, idot(mpg, gear, labels = labels))
```
...
```{r}
with(ivolcano,
     idot(logFC, substr(rownames(ivolcano), 1, 1), subgroup = pval < 0.05,
...
Each type of plot has its own set of options and customizations that affect the appearance of individual aesthetic elements. Some options such as `pointcolor`, `pointsize`, or `rectcolor` are common among all or most plots, but others apply only to specific plots. See `?iplotOpts` for a list of the plots in this package as well as links to the lists of options for each plot.
